...
def evaluate(data, test_data, config):
	...
	if config.uncertainty and len(config.model_types) > 1:
		print('Test Average, Std Uncertainties: ', list(np.mean(average_uncertainties, axis=0)), list(np.std(average_uncertainties, axis=0)))
		print('Test Average Entropies: ', list(np.mean(average_entropies, axis=0)))
		print('Test All Uncertainties: ', list(np.mean(average_uncertainties, axis=0)))
...
def get_ensemble_accuracy(task, models, features, y, config, num_classes=2, learnt_voter=None, fold=None, plot=False):
	...
	if task == 'classification':
		...
		if plot:
			...
			if 'emp_plots' not in os.listdir(config.model_dir):
				os.mkdir(os.path.join(config.model_dir, 'emp_plots'))
			if config.voting_type not in os.listdir(os.path.join(config.model_dir, 'emp_plots')):
				os.mkdir(os.path.join(config.model_dir, 'emp_plots', config.voting_type))
			np.save(os.path.join(config.model_dir, 'emp_plots', config.voting_type, 'voted_predictions_fold_{}'.format(fold)), voted_predictions)
			np.save(os.path.join(config.model_dir, 'emp_plots', config.voting_type, 'pred_stds_fold_{}'.format(fold)), pred_stds)
			np.save(os.path.join(config.model_dir, 'emp_plots', config.voting_type, 'y_fold_{}'.format(fold)), y)
